...
public class ChinaHat extends Module {
    ...
    public void onRender3DEvent(Render3DEvent event) {
        ...
        for (EntityPlayer player : mc.theWorld.playerEntities) {
            ...
            float yaw = (float) MathUtils.interpolate(player.prevRotationYawHead, player.rotationYawHead, partialTicks);
            if (Tenacity.INSTANCE.isEnabled(KillAura.class) || Tenacity.INSTANCE.isEnabled(Scaffold.class)) {
                yaw = (float) MathUtils.interpolate(MathHelper.wrapAngleTo180_float(player.prevRotationYawHead),
                        MathHelper.wrapAngleTo180_float(player.rotationYawHead), partialTicks);
            }


            float pitch = (float) MathUtils.interpolate(player.prevRotationPitchHead, player.rotationPitchHead, partialTicks);
